...
_tls_generate_certs() {
	local dir="$1"; shift
	echo "Generating TLS certificates in $dir"
	...
	if [ -s "$dir/server/ca.pem" ] && [ -s "$dir/server/cert.pem" ] && [ -s "$dir/server/key.pem" ] && [ ! -s "$dir/ca/key.pem" ]; then
		echo "Verifying existing server certificates (user-managed CA)"
	...
	if [ -s "$dir/ca/key.pem" ] || [ ! -s "$dir/ca/cert.pem" ]; then
		...
		echo "Creating/updating CA certificate"
		mkdir -p "$dir/ca"
	...
	if [ -s "$dir/ca/key.pem" ]; then
		...
		echo "Creating/updating server certificate"
		mkdir -p "$dir/server"
	...
	if [ -s "$dir/ca/key.pem" ]; then
		...
		echo "Creating/updating client certificate"
		mkdir -p "$dir/client"
...
if [ "$#" -eq 0 ] || [ "${1#-}" != "$1" ]; then
	echo "Setting up default dockerd configuration"
	...
	if [ "$uid" = '0' ]; then
		...
		echo "Running as non-root user (rootless mode)"
	...
	if [ -n "${DOCKER_TLS_CERTDIR:-}" ]; then
		echo "TLS enabled, configuring for port 2376"
		...
		echo "TLS disabled, configuring for port 2375"
...
if [ "$1" = 'dockerd' ]; then
	echo "Preparing to start dockerd"
	...
	if [ -n "${DOCKER_IPTABLES_LEGACY+x}" ]; then
		...
		if [ -n "$iptablesLegacy" ]; then
			echo "Using legacy iptables (explicitly configured)"
			modprobe ip_tables || :
			modprobe ip6_tables || :
		else
			echo "Using nftables (explicitly configured)"
			modprobe nf_tables || :
		fi
	elif (
		...
		echo "Detected existing legacy iptables rules, using legacy iptables"
		iptablesLegacy=1
	...
	if [ -n "$iptablesLegacy" ]; then
		...
		echo "Setting PATH for legacy iptables"
		export PATH="/usr/local/sbin/.iptables-legacy:$PATH"
	fi
	echo "Using iptables version:"
	iptables --version # so users can see whether it's legacy or not
	uid="$(id -u)"
	if [ "$uid" != '0' ]; then
		# if we're not root, we must be trying to run rootless
		echo "Checking rootless mode prerequisites"
		...
		echo "Starting rootless dockerd with rootlesskit"
		...
		echo "Using legacy dind wrapper"
	...
	echo "Not running dockerd, passing through to docker-entrypoint.sh"

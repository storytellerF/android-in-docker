...
# Install Docker runtime dependencies (Ubuntu)
RUN set -eux; \
    apt-get update; \
...
# Ubuntu-specific packages (nftables is default in Ubuntu, these packages may not exist in newer versions)
RUN set -eux; \
    apt-get update; \
...
# Configure iptables-legacy fallback for kernels without nf_tables
RUN set -eux; \
    ...
    if command -v iptables-legacy > /dev/null 2>&1; then \
        for f in iptables iptables-save iptables-restore ip6tables ip6tables-save ip6tables-restore; do \
            case "$f" in \
                iptables) b="$(command -v iptables-legacy)" ;; \
                iptables-save) b="$(command -v iptables-legacy-save)" ;; \
                iptables-restore) b="$(command -v iptables-legacy-restore)" ;; \
                ip6tables) b="$(command -v ip6tables-legacy)" ;; \
                ip6tables-save) b="$(command -v ip6tables-legacy-save)" ;; \
                ip6tables-restore) b="$(command -v ip6tables-legacy-restore)" ;; \
                *) b="" ;; \
            esac; \
            if [ -n "$b" ]; then \
...
RUN set -eux; \
    ...
    useradd -r -g dockremap -s /sbin/nologin dockremap; \
...
RUN set -eux; \
    ...
    case "$debArch" in \
        ...
        'arm64') \
            ...
            ;; \
        *) echo >&2 "error: unsupported 'docker.tgz' architecture ($debArch)"; exit 1 ;; \
    esac; \
    \
...
RUN set -eux; \
    ...
    case "$debArch" in \
        ...
        'arm64') \
            ...
            ;; \
        *) echo >&2 "warning: unsupported 'docker-buildx' architecture ($debArch); exit 1" ;; \
    esac; \
    \
...
RUN set -eux; \
    ...
    case "$debArch" in \
        ...
        'arm64') \
            ...
            ;; \
        *) echo >&2 "warning: unsupported 'docker-compose' architecture ($debArch); exit 1" ;; \
    esac; \
    \

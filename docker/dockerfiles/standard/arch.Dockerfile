...
RUN pacman -Sy --noconfirm --needed bash ca-certificates curl tar xz \
...
RUN mkdir -p "$NVM_DIR" \
    ...
    && bash -lc '. "$NVM_DIR/nvm.sh" && nvm install node && nvm alias default node && node_version="$(nvm version default)" && ln -sfn "$NVM_DIR/versions/node/$node_version" "$NVM_DIR/current" && node --version && npm --version' \
